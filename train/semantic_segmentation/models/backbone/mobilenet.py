...
class MobileNetV2(nn.Module):
    def __init__(
        ...
        self.features = [i.to("cuda") for i in self.features]
    ...
    def forward(self, x):
        # print("input", x.shape)
        for feat in self.low_level_features:
            x = feat(x)
            # print(x.shape)

        low_level_feat = x
        jpu_feats = []
        for feat in self.high_level_features:
            x_ = feat(x)
            if x_.shape[-1] != x.shape[-1]:
                jpu_feats.append(x)
            x = x_
        jpu_feats = jpu_feats[1:]
        # print([i.shape for i in jpu_feats], x.shape)
        ...
        # print(x.shape)
        return x, [low_level_feat, jpu_feats]
